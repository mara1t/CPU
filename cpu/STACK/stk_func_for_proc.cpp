...
{
    FILE* canary_file = fopen("errors_stack.txt", "w");
...
data_type MurMurHash (Stack* stk)
{
    ...
    else
    {
        for (int counterj = 0; counterj < stk->size; counterj++)
        {
            ...
            sum += (stk->data[counterj]) * (counterj | 30);
        }
